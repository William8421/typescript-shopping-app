...
export default function ShoppingCart({ isOpen }: ShoppingCartProps) {
  const { openCloseCart, cartItems, emptyCart, calculateTotal } =
    useShoppingCart();
    ...
    <div>
      ...
      <div className={`off-canvas ${isOpen}`}>
        <div className="off-canvas-header">
          <h2>Cart</h2>
          <button className="close-button" onClick={openCloseCart}>
            X
          </button>
        </div>
        <div className="items-container">
          ...
          <div className="total">
            ...
            {cartItems.length !== 0 ? (
              <div>
                <button className="main-button" onClick={() => emptyCart()}>
                  Empty Cart
                </button>
                <button className="main-button" onClick={navigateToCheckout}>
                  Check Out
                </button>
              </div>
            ) : (
